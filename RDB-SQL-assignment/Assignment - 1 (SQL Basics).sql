...
/*
We can say that we did something like grouping because the first command read
was brand_id and there were not many unique values in brand_id. But in the next one,
since we sorted by list_price first, the ordering we did with brand_id no longer matters.
Because there are more unique values in list_price.
*/
